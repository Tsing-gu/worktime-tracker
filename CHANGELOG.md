...
- **修复**: 统一管理后台任务的启动与退出，关闭应用时等待线程结束，避免 Python 意外退出
- **修复**: 关闭工时回溯窗口时可终止 pmset 子进程，不再向已销毁窗口发送更新

## [0.17.3] - 2026-07-31

- **修复**: 加固自动更新、网络检测与跨天工时补录，避免不安全连接、错误未来时间和异常超长工时
- **修复**: 更新取消、安装失败和并发检查异常不会破坏原应用或重复弹窗

## [0.17.2] - 2026-07-30

- **新增**: 日历弹窗新增「工时回溯」功能，可从系统日志回溯近 7 天上下班时间并选择性应用

## [0.17.1] - 2026-07-30

- **修复**: 回退设置页下拉列表改造恢复原有输入控件，时间控件强制 24 小时制显示避免跟随系统区域设置

## [0.17.0] - 2026-07-30

- **变更**: 设置页时间与数值字段改为下拉列表与带校验的输入框，提升输入体验；修复下班判定在重启后无法恢复计时、关机后跨天补录时间错误的问题

## [0.16.4] - 2026-07-30

- **修复**: 修复按钮 hover 视效不跟手、点击有 ~0.2 秒延迟、点击无视觉反馈的问题（按钮单独应用 Fusion style，避免 macOS 原生按钮绘制动画）
- **修复**: 修复托盘右键菜单与左键时长卡切换时上一个不消失的问题
- **变更**: 所有弹窗改为默认窗口模式，显示在父窗口前方、有标准标题栏关闭按钮，关闭无残影

## [0.16.3] - 2026-07-30

- **修复**: 彻底修复所有对话框按钮（含消息框）首次点击被吞的问题，统一按钮封装为单实例模式

## [0.16.2] - 2026-07-30

- **修复**: 修复对话框按钮点击后焦点跳走、需要点击两次才生效的问题

## [0.16.1] - 2026-07-29

- **修复**: 修复部分按钮偶尔需要点击两次才能生效的问题

## [0.16.0] - 2026-07-29

- **变更**: 重构代码架构分层，完善工程基础设施（类型检查、单元测试、日志系统），提升程序稳定性与可维护性

## [0.15.3] - 2026-07-29

- **修复**: 修复首次使用时默认勾选「只记录在公司时间」但未配置办公网络，导致到达下班时间后频繁误判下班的问题
